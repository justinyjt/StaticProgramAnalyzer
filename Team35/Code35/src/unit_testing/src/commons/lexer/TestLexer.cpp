...
TEST_CASE("1. Lexer test basic token") {
    ...
    token = lexer.scan();
    requireEqual(*token, Token("p", Token::Tag::Name));
    token = lexer.scan();
    requireEqual(*token, Token("{", Token::Tag::LBrace));
    token = lexer.scan();
    requireEqual(*token, Token("x", Token::Tag::Name));
    token = lexer.scan();
    requireEqual(*token, Token("=", Token::Tag::Assign));
    token = lexer.scan();
...
TEST_CASE("2. Test new line in token") {
    ...
    requireEqual(*token, Token("=", Token::Tag::Assign));
    token = lexer.scan();
    requireEqual(*token, Token("2", Token::Tag::Integer));
    token = lexer.scan();
    requireEqual(*token, Token(";", Token::Tag::SemiColon));
    token = lexer.scan();
    requireEqual(*token, Token("call", Token::Tag::Call));
    token = lexer.scan();
    requireEqual(*token, Token("q", Token::Tag::Name));
    token = lexer.scan();
    requireEqual(*token, Token(";", Token::Tag::SemiColon));
    token = lexer.scan();
    requireEqual(*token, Token("}", Token::Tag::RBrace));
