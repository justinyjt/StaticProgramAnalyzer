...
TEST_CASE("Selection parser") {
    ...
    requireTrue(*sc == *std::make_unique<MultipleSynonymSelectClause>(std::move(s)));
}
...
TEST_CASE("Selection parser, attrRef") {
    ...
    std::unique_ptr<SelectClause> expected = std::make_unique<MultipleSynonymSelectClause>(std::move(attrRef));
