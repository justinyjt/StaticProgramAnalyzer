...
std::unique_ptr<SelectClause> SelectionParser::parse() {
    ...
    if (pqlTokenScanner.peek(Token::Tag::Bool)) {  // BOOLEAN
        if (!SemanticValidator::isDeclared(synonyms, "BOOLEAN")) {
            ...
            std::unique_ptr<SelectClause> selectClause = std::make_unique<MultipleSynonymSelectClause>(std::move(selectedSynonyms));
            return selectClause;
        }
    } else if (pqlTokenScanner.peek(Token::Tag::LessThan)) {
        pqlTokenScanner.match(Token::Tag::LessThan);
        std::unique_ptr<SelectClause> selectClause = std::make_unique<MultipleSynonymSelectClause>(std::move(parseMultiSelect()));
        return selectClause;
