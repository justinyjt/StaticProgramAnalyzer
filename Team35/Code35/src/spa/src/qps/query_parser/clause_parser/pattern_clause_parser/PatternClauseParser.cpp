...
std::unique_ptr<Clause> PatternClauseParser::parse(TokenValidator &tokenValidator, std::vector<Synonym> synonyms) {
    ...
    std::unique_ptr<Clause> clause = createClause(std::move(leftArg), std::move(rightArg),
...
std::unique_ptr<Clause> PatternClauseParser::createClause(std::unique_ptr<Token> token1,
    ...
    std::shared_ptr<PQLToken> first = createLeftArg(token1, synonyms);
    std::shared_ptr<PQLToken> second = createRightArg(token2);
    if (isEntRef(*first)) {
        std::unique_ptr<Clause> a = std::make_unique<Pattern>(first, second, patternStr);
        return std::move(a);
    }
    throw SyntaxException();
}
